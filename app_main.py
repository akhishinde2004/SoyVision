...
def analyze_image(filename):
    ...
    if real_size < 1024:
        size_str = f"{real_size} B"
    elif real_size < 1024 * 1024:
        size_str = f"{real_size/1024:.1f} KB"
    else:
        size_str = f"{real_size/(1024*1024):.1f} MB"
...
def register():
    ...
    if request.method == 'POST':
        ...
        if user_id:
            ...
            flash('Registration successful! Welcome to Soyabean Leaf Disease Detection.', 'success')
            return redirect(url_for('index'))
        else:
